...
$services = $servicesTable->find()->where(['status' => 1])->order(['position' => 'ASC'])->all();
?>
...
<div class="estimateForm_wrap">
    ...
    <div class="row g-3">
        ...
        <div class="col-12">
            <h5 style="font-size: 17px;line-height: 1.4;color: #fff;margin-bottom: 4px;">Type of Property :</h5>
            <div class="d-flex gap-4" role="group" aria-labelledby="property-type-label" aria-describedby="property-type-error">
                <div id="property-type-label" class="sr-only">Property Type Selection</div>
                <div class="checkItem">
                    <input type="radio" id="propertyTypeResidential" name="property_type" value="1" data-validation-type="property_type" aria-invalid="false" aria-required="true" required>
                    <label for="propertyTypeResidential">Residential</label>
                </div>
                <div class="checkItem">
                    <input type="radio" id="propertyTypeBusiness" name="property_type" value="2" data-validation-type="property_type" aria-invalid="false" aria-required="true" required>
                    <label for="propertyTypeBusiness">Business</label>
                </div>
            </div>
            <div id="property-type-error" class="error-message" role="alert" aria-live="polite"></div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        propertyTypeInputs.forEach(input => {
            input.addEventListener('change', function() {
                ...
                propertyTypeError.classList.remove('show');
                propertyTypeInputs.forEach(radio => {
            ...
            if (isRequired && !value) {
                ...
                switch (fieldType) {
                    case 'property_type':
                        const propertyTypeInputs = document.querySelectorAll('input[name="property_type"]');
                        const propertyTypeError = document.getElementById('property-type-error');
                        const isChecked = document.querySelector('input[name="property_type"]:checked');

                        if (!isChecked) {
                            isValid = false;
                            errorMessage = 'Please select a property type';
                            // Set aria-invalid on all radio buttons in the group
                            propertyTypeInputs.forEach(input => {
                                input.setAttribute('aria-invalid', 'true');
                            });
                            // Show error message
                            propertyTypeError.textContent = errorMessage;
                            propertyTypeError.classList.add('show');
                        } else {
                            // Clear error state
                            propertyTypeInputs.forEach(input => {
